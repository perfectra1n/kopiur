...
the admission webhook, the 7 `kopiur.home-operations.com/v1alpha1` CRDs, and the RBAC to run them.
...
`deploy/rbac/operator-*.yaml`), which derives the `kopiur.home-operations.com` permissions from the
...
**and every `kopiur.home-operations.com` object in the cluster**. For decoupled CRD lifecycle (e.g.
...
that turns everything on is at `deploy/observability-values.yaml`. The
