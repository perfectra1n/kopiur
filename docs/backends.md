...
credential-key conventions), then jump to your backend below. For the *concepts*
...
|---|---|---|
| [S3 / S3-compatible](backends/s3.md) | `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, *(opt)* `AWS_SESSION_TOKEN` | `backend.s3` |
...
| [Filesystem](backends/filesystem.md) | *(none — only `KOPIA_PASSWORD`)* | `backend.filesystem` |
...
| [rclone](backends/rclone.md) | `KOPIA_RCLONE_CONFIG` *(via `configSecretRef`)* | `backend.rclone` |
