...
```yaml
{{#include ../../deploy/examples/backends/azure-sas.yaml}}
```
...
```admonish warning title="Provide exactly one credential"
...
- **`AuthenticationFailed`** — wrong key, expired SAS token, or a SAS scoped to the
  wrong container. Regenerate scoped to *this* container.
