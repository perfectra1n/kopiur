...
credentials *file*, and the SDK env var `GOOGLE_APPLICATION_CREDENTIALS` holds a
*path*, not the JSON — so Kopiur reads the JSON body from a well-known env key and
...
```admonish info title="Why KOPIA_GCS_CREDENTIALS and not GOOGLE_APPLICATION_CREDENTIALS"
...
```admonish warning title="Paste the JSON body, not a path"
