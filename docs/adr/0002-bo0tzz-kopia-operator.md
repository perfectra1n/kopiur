...
| **`BackupConfig`**   | Recipe               | _What_ to back up: PVC selector, identity, retention, policy, hooks. Idempotent — doesn't run anything on its own.                                                                                                                             |
...
| **`BackupSchedule`** | Cron                 | _When_ it runs: cron (with jitter + timezone) + `configRef`. Creates `Backup` CRs.                                                                                                                                                             |
